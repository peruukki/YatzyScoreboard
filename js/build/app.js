...
(function () {
  ...
  var UPPER_SECTION_INPUTS = [Input('Ones', [1], 1, 5), Input('Twos', [2], 2, 10), Input('Threes', [3], 3, 15), Input('Fours', [4], 4, 20), Input('Fives', [5], 5, 25), Input('Sixes', [6], 6, 30)];
  var UPPER_SECTION_TOTAL_LABEL = 'Total';

  var LOWER_SECTION_INPUTS = [Input('Pair', [6, 6], 2, 12), Input('Two pairs', [6, 6, 5, 5], 2, 24), Input('Three of a kind', [6, 6, 6], 3, 18), Input('Four of a kind', [6, 6, 6, 6], 4, 24), Input('Five of a kind', [6, 6, 6, 6, 6], 5, 30), Input('Small straight', [1, 2, 3, 4, 5], 15, 15), Input('Large straight', [2, 3, 4, 5, 6], 20, 20), Input('Full house', [6, 6, 6, 5, 5], 1, 30), Input('Chance', [6, 6, 5, 4, 3], 1, 30), Input('Yatzy', [6, 6, 6, 6, 6], 5, 80)];
  ...
  function addBodyRows(tableBodyElement, columnHeaders, rowHeaders, totalLabel) {
    ...
    $(tableBodyElement).append($('<tr class="total">').append('<th>' + totalLabel + '</th>').append(columnHeaders.map(function (columnHeader, index) {
  ...
  function bindNameData(sourceElement, targetElement) {
    Rx.Observable.fromEvent(sourceElement, 'keyup').map(function (e) {
      return e.target.value;
    }).distinctUntilChanged().subscribe(function (text) {
  ...
  function getInputObservablesByTableColumn(tableSelector) {
    var inputsByColumn = _($(tableSelector + ' input.score').get()).groupBy(function (value, index) {
      return index % COLUMN_HEADERS.length;
    }).valueOf();
  ...
  function createObservableForNumberInputField(inputElement) {
    return Rx.Observable.just(0).merge(Rx.Observable.fromEvent($(inputElement), 'input').map(function (e) {
  ...
  function bindRowScoreData(tableSelector, scoreObservables) {
    $(tableSelector + ' tbody tr').each(function (rowIndex, rowElement) {
      var scoreObservablePairs = _(scoreObservables).groupBy(function (value, index) {
        return Math.floor(index / 2);
      }).toArray();
      scoreObservablePairs.forEach(function (scoreObservablePair, pairIndex) {
        var rowScoreChangeObservable = Rx.Observable.combineLatest(scoreObservablePair[0][rowIndex], scoreObservablePair[1][rowIndex], function (score1, score2) {
          return RowScoreChange(score1, score2);
        });

        var score1MinusScore2 = function score1MinusScore2(change) {
          return change.score1 - change.score2;
        };
        var score2MinusScore1 = function score2MinusScore1(change) {
          return change.score2 - change.score1;
        };
        var isScore1Bigger = function isScore1Bigger(change) {
          return score1MinusScore2(change) > 0;
        };
        var isScore2Bigger = function isScore2Bigger(change) {
          return score2MinusScore1(change) > 0;
        };
        var not = function not(comparisonFunc) {
          return function (change) {
            return !comparisonFunc(change);
          };
        };

        // Show score difference next to bigger score cell
        rowScoreChangeObservable.filter(isScore1Bigger).subscribe(showScoreDifference(rowElement, '.score-diff.player-1', pairIndex, score1MinusScore2));
        rowScoreChangeObservable.filter(isScore2Bigger).subscribe(showScoreDifference(rowElement, '.score-diff.player-2', pairIndex, score2MinusScore1));

        // Hide score difference cell next to score cell that is smaller or equal to the other one
        rowScoreChangeObservable.filter(not(isScore1Bigger)).subscribe(hideScoreDifference(rowElement, '.score-diff.player-1', pairIndex));
        rowScoreChangeObservable.filter(not(isScore2Bigger)).subscribe(hideScoreDifference(rowElement, '.score-diff.player-2', pairIndex));
      });
    });
  }
  ...
  function showScoreDifference(rowElement, diffElementSelector, diffElementIndex, scoreDiffCalculator) {
    return function (change) {
      $(rowElement).find(diffElementSelector).slice(diffElementIndex, diffElementIndex + 1).addClass('visible').html('+' + scoreDiffCalculator(change));
    };
  }

  function hideScoreDifference(rowElement, diffElementSelector, diffElementIndex) {
    return function () {
      $(rowElement).find(diffElementSelector).slice(diffElementIndex, diffElementIndex + 1).removeClass('visible');
    };
  }

  function observeElementsHiddenAfterTransition() {
    $('.score-diff').each(function (index, element) {
      Rx.Observable.fromEvent(element, 'transitionend').filter(function (e) {
        return e.propertyName === 'opacity';
      }).filter(function (e) {
        return !$(e.target).hasClass('visible');
  ...
  function addResetButtons(tableSelector, buttonCount) {
    ...
    $(tableSelector + ' tbody').append($('<tr>').append('<th>').append(buttonCells));

    // Add click event handlers to clear score inputs
    $(tableSelector + ' td.reset .button').each(function (index, element) {
      Rx.Observable.fromEvent(element, 'click').subscribe(function () {
        $('td.game-' + (index + 1) + ' input.score').val('').trigger('input');
      });
    });
  }
})();
